...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
    ...
    if trainer_name == 'nnUNetTrainer':
        ...
        nnunet_trainer = nnunet_trainer(plans=plans, configuration=configuration, fold=fold,
                                        dataset_json=dataset_json, unpack_dataset=not use_compressed, device=device,
                                        aug=aug, max_epochs=max_epochs, test=test)
...
def run_training(dataset_name_or_id: Union[str, int],
    ...
    if num_gpus > 1:
        ...
        nnunet_trainer = get_trainer_from_args(dataset_name_or_id, configuration, fold, trainer_class_name,
                                               plans_identifier, use_compressed_data, device=device, aug=aug,
                                               max_epochs=max_epochs, test=test)
...
def run_training_entry():
    ...
    parser.add_argument('-aug', type=str, default=None, required=False)
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights,
                 ...
                 device=device, aug=args.aug, max_epochs=args.max_epochs, test=args.test)


if __name__ == '__main__':
    run_training_entry()
    # run_training('Dataset001_TorsionHip', '3d_fullres', 'all', 'nnUNetTrainerCustom', continue_training=True)
